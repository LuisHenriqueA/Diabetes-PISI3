...
def build_header():
    text = (
        '<h1>Gráficos com base de diabetes</h1>'
...
def build_body():
    ...
    build_boxplot_expander(df, df_bruto)
...
def build_diabetesplot_section(df: pd.DataFrame):
    ...
    fig.update_layout(
        title=f'Histograma de {selec_col} por condição diabética.',
...
def create_histograma_stacked(df: pd.DataFrame, series_col: str) -> go.Figure:
    ...
    return px.histogram(
        df, x='Diabetes', nbins=20, color=series_col, opacity=.75, color_discrete_map=color_map
    )
...
def build_boxplot_expander(df: pd.DataFrame, df_bruto: pd.DataFrame):
    ...
    with st.expander('Heatmap de condição diabética por Saúde geral'):
        ...
        heatmap_data = df_filtered.pivot_table(
            index='Saúde_geral', columns='Diabetes', aggfunc='size', fill_value=0
        )
        fig1 = px.imshow(
            heatmap_data, 
